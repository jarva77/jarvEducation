...
- Ο χρήστης δουλεύει με **Visual Studio** (όχι VS Code). Ο φάκελος `.vs/` είναι στο
  `.gitignore` (από 2026-09-03) — ό,τι γράφει εκεί το VS δεν είναι αλλαγή και δεν γίνεται
  ποτέ commit.

## Τράπεζα ερωτήσεων (ΣΗΜΑΝΤΙΚΟ WORKFLOW)

- 6 τάξεις (Α'–ΣΤ'): `src/data/questions-{a,b,c,d,e,f}.json`. Μαθηματικά/Γραμματική/Ορθογραφία/Μελέτη
  Περιβάλλοντος υπάρχουν σε όλες (500 ερωτήσεις). Από τη Γ' και μετά προστίθενται μαθήματα
  ανάλογα με το πρόγραμμα σπουδών, ΠΑΝΩ στο 500 (όχι αφαιρετικά): Γ'/Δ' = 600 (+Ιστορία,
  +Θρησκευτικά), Ε'/ΣΤ' = 700 (+Ιστορία, +Θρησκευτικά, +Γεωγραφία, +Φυσικές Επιστήμες). Α'/Β'
  μένουν στα 500 — αυτά τα μαθήματα δεν διδάσκονται ακόμα σε αυτές τις τάξεις.
...
## Τεκμηρίωση & συμβάσεις έργου

Από 2026-09-07 το project ακολουθεί το cross-project engineering operating
model (ίδιο με το sibling repo `geoQuiz`), **προσαρμοσμένο** στις συμβάσεις που
ήδη ίσχυαν εδώ — όπου συγκρούονταν, υπερίσχυσε η υπάρχουσα σύμβαση. Πριν
αλλάξεις οτιδήποτε, να ξέρεις πού πάει το κάθε τι:

- **`docs/todo.md` / `docs/todo.archive.md`** — ανοιχτά και κλεισμένα work items. Σταθερά
  `#N` ids που δεν ξαναχρησιμοποιούνται· το κλείσιμο μετακινεί το item αυτούσιο
  στο archive με ημερομηνία, δεν το διαγράφει.
- **`docs/adr/`** — ένα αρχείο ανά σημαντική απόφαση, αμετάβλητο μόλις γίνει
  accepted. Η αναστροφή είναι νέο ADR, ποτέ ξαναγράψιμο του παλιού. Κανόνες και
  ευρετήριο στο `docs/adr/README.md`.
- **`docs/engineering-log.md`** — το πλήρες γιατί/πότε/πώς, ζωντανό και
  επεξεργάσιμο επιτόπου. Μόνο εσωτερικό· δεν φτάνει ποτέ σε χρήστη.
- **`docs/conventions.md`** — οι κανόνες που δεν είναι αρχεία. Περιέχει
  **αυτούσιες** τις υπάρχουσες συμβάσεις αυτού του repo (παραγόμενα JSON,
  κανόνες ποιότητας ερωτήσεων, `npm run build` + 375px πριν από push, `base:
  './'`, `.vs/`, lazy-loaded Firebase SDK, αποφάσεις προϊόντος) και επιπλέον τον
  κανόνα έκδοσης, τι σημαίνει το `main`, τον διαχωρισμό status/delta εγγράφων,
  και την απαίτηση κάθε test layer να δικαιολογείται από ονομασμένο δομικό κενό
  πριν προστεθεί.

Το `main` **είναι ό,τι είναι ζωντανό**: το GitHub Actions workflow το χτίζει και
το δημοσιεύει σε κάθε push, χωρίς πύλη ενδιάμεσα — και η εφαρμογή τη
χρησιμοποιούν παιδιά, με ζωντανό Firestore και δημόσια Βαθμολογία.

## Συμβάσεις

Συνοπτικά· η πλήρης λίστα με το σκεπτικό της κάθε μιας είναι στο
`docs/conventions.md`.

- Πριν από push: `npm run build` (τρέχει και vue-tsc). Οπτικός έλεγχος σε mobile viewport (375px) στο preview.
- Commits: αγγλικά, τελειώνουν με γραμμή `Co-Authored-By: <μοντέλο> <noreply@anthropic.com>` — το όνομα είναι του μοντέλου που έκανε τη δουλειά (στο ιστορικό υπάρχουν ήδη `Claude Fable 5`, `Claude Fable 5.1`, `Claude Sonnet 5`).
- Επικοινωνία με τον χρήστη στα ελληνικά. Η τεκμηρίωση του repo (`CLAUDE.md`, `README.md`, `docs/`) επίσης στα ελληνικά.
