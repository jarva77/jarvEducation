...
onMounted(() => {
  ...
  timers.push(window.setTimeout(() => (showText.value = true), 900))
...
<template>
  <div class="intro-splash" :class="{ closing }" @click="finish">
    <div class="intro-brain" :class="{ open: opening }">
    ...
    <p class="intro-text" :class="{ visible: showText }">open your mind</p>
  </div>
</template>
